...
# Prediction Section
    ...
    try:
        ...
        # Add sliders for numerical inputs with transformed values
        for col in numerical_features:
            if col in data.columns:
                # Transform values for better clarity
                if col in ['store_sales(in millions)', 'store_cost(in millions)']:
                    min_val = int(data[col].min() * 1_000)  # Convert millions to thousands
                    max_val = int(data[col].max() * 1_000)
                    mean_val = int(data[col].mean() * 1_000)
                    step = 100  # Step by hundreds for better adjustment
                else:
                    min_val = int(data[col].min())
                    max_val = int(data[col].max())
                    mean_val = int(data[col].mean())
                    step = 10  # Step by tens for better adjustment
                ...
                input_data[col] = st.slider(
                    ...
                    value=mean_val,
                    step=step,
                    format="%d"  # Display as whole numbers
                )
        ...
        if st.button("Predict"):
            ...
            # Convert back transformed inputs to original scale
            for col in ['store_sales(in millions)', 'store_cost(in millions)']:
                if col in input_df.columns:
                    input_df[col] = input_df[col] / 1_000  # Convert back to millions
            ...
            try:
                ...
                st.subheader("Input Values Used")
                for key, value in input_data.items():
                    if key in ['store_sales(in millions)', 'store_cost(in millions)']:
                        st.write(f"{key.replace('(in millions)', '(in thousands)')}: {value:,d} (in thousands)")
                    else:
                        st.write(f"{key}: {value:,d}")
            except Exception as e:
                st.error(f"Prediction error: {str(e)}")
